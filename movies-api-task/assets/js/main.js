...
function renderCards(arr) {
  ...
  arr.forEach((movie) => {
    moviesWrapper.innerHTML += `<div class="col-lg-3 col-md-6 col-sm-12" data-id=${movie.id} data-editing="false">
        <div class="card">
           ...
           ${!localData?.admin ? `<button  class="btn heartBtn  btn-outline-danger"><i id=${movie.id} ${movie.heart? `class="fa-solid fa-heart"`:`class="fa-regular fa-heart"`} ></i></button>` : ""}
            </div>
        </div>
    </div>`;
    ...
    elements.forEach(element => {
      ...
      if (element.id == favor.moviID) {
        ...
        element.addEventListener("click", function () {
          ...
          arr.forEach(movies=>{
            if(movies.id == favorIdToDelete){
              movies.heart = !movies.heart;
              fetch(`http://localhost:3000/movies/${favorIdToDelete}`, {
                method: 'PATCH',
          ...
          fetch(`http://localhost:3000/users/${localData.id}`, {
            method: 'PATCH',
            body: JSON.stringify({
              favorites: localDAta2.favorites.filter(favorite => favorite.moviID != favorIdToDelete)
            }),
            headers: {
              "Content-type": "application/json; charset=UTF-8"
            }
  ...
  function sortByFavs(favs) {
    getAll(endpoints.movies).then(res => {
      if (favs === "favorites") {
        const favoriteMovie = res.data.filter(movie => movie.heart === true)
  ...
  btnHeartAll.forEach(btnHeart => {
    btnHeart.addEventListener("click", function () {
      ...
      if (this.children[0].className === "fa-solid fa-heart") {
        ...
        this.children[0].id = moviID;
        arr.forEach(movies=>{
          if(movies.id == moviID){
            movies.heart = !movies.heart;
            fetch(`http://localhost:3000/movies/${moviID}`, {
              method: 'PATCH',
              body: JSON.stringify(movies),
              headers: {
                "Content-type": "application/json; charset=UTF-8"
